...
def set_frame_processors_modules_from_ui(frame_processors: List[str]) -> None:
    ...
    for frame_processor, state in modules.globals.fp_ui.items():
        if state and frame_processor not in frame_processors:
            module_name = f'modules.processors.frame.{frame_processor}'
            if importlib.util.find_spec(module_name) is None:
                modules.globals.fp_ui[frame_processor] = False
                continue
            frame_processor_module = load_frame_processor_module(frame_processor)
            if frame_processor_module not in FRAME_PROCESSORS_MODULES:
                FRAME_PROCESSORS_MODULES.append(frame_processor_module)
            modules.globals.frame_processors.append(frame_processor)
        elif not state:
            module_name = f'modules.processors.frame.{frame_processor}'
            FRAME_PROCESSORS_MODULES[:] = [
                module for module in FRAME_PROCESSORS_MODULES
                if module.__name__ != module_name
            ]
            while frame_processor in modules.globals.frame_processors:
